...
Set 30 "infected" individuals loose to move through the network from a single initial hospital location, and note the first time that each hospital is visited by one of these individuals.
...
Comparing median observed extent between different models, paired by `seed' hospital.

```{python}
#| label: fig-pairplot
#| fig-cap: Pairplot of median extent, where each point is a seed location
def compute_median_extent(df: pl.DataFrame):
    return df.group_by('seed').median().select('seed', 'extent')

combined_median_df = pl.concat(
    [compute_median_extent(df).with_columns(pl.lit(label.replace(' ', '\n')).alias('simulation'))
     for label, df in processed_results.items()]
)
...
ppl = sns.PairGrid(paired_median_df.to_pandas(), vars=combined_median_df.select(pl.col('simulation').unique()).to_series().to_list(), height=2)
ppl.map_lower(sns.scatterplot, marker='.')
