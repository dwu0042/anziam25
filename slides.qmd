...
Treat hospitals as nodes of a static network
...
\begin{tikzpicture}[scale=2, transform shape]
  ...
  \draw[->] (A) -- (C) node[midway,below left] {$(1-p)(1-q) \lambda_{ij}(t)$};
  \draw[->] (C) -- (B) node[midway,below right] {$\tau_{ij}(t)$};
  \draw[->] (A) -- (Z) node[midway,above left] {$\lambda_{i}(t)$};
  \draw[->] (A) -- (B) node[midway,above] {$(1-p)q \lambda_{ij}(t)$};
\end{tikzpicture}
```
...
- $q$: probability of direct transfer
- $\lambda_{ij}$: rate of movement from hospital $x_i$ to hospital $x_j$
- $\tau_{ij}$: rate of return from home $z_{ij}$ to hospital $x_j$

## Models of Movement

The static network can be considered as a model where

- probability of direct transfer $q$ is $1$
- rate of movement between hospitals $\lambda_{ij}$ is static over all time
- rate of return from homes $\tau_{ij}$ is static over all time
...
The inter-admission time (characterised by $\tau$) is poorly represented by a single exponential random variable.

We can improve this by allowing individuals to go home (allow for indirect transfers)
